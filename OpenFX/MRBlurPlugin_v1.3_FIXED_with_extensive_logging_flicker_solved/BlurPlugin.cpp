    ...
    : ImageEffect(p_Handle)
{
    ...
    if (!g_LogInitialized) {
        g_LogFile = fopen("/tmp/blur_plugin_log.txt", "w");
        if (g_LogFile) {
            fprintf(g_LogFile, "=== New BlurPlugin Session Started ===\n");
            fflush(g_LogFile);  // Flush but don't close
            ...
            fprintf(g_LogFile, "Session time: %s\n", buffer);
            fflush(g_LogFile);
        }
    }
    
    // Log instance creation
    logMessage("BlurPlugin instance created");
...
{
    // destructor
    // Only close the file if this is the last instance
    if (g_LogFile) {
        fprintf(g_LogFile, "=== BlurPlugin Instance Destroyed ===\n");
        fclose(g_LogFile);
        g_LogFile = NULL;
...
void BlurPlugin::logMessage(const char* format, ...)
{
    if (g_LogFile) {
        va_list args;
