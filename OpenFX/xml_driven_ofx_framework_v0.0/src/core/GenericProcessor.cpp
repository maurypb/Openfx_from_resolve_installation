...
void GenericProcessor::callDynamicKernel(const std::string& platform) {
    ...
    // For now, call the existing blur kernel as a test
    // TODO: This will be made dynamic based on effect name
    std::string effectName = m_xmlDef.getName();
    Logger::getInstance().logMessage("Effect name: %s", effectName.c_str());
    
    if (effectName.find("Blur") != std::string::npos) {
        // Extract parameters (with defaults if missing)
        float radius = 5.0f;
        int quality = 8;
        float maskStrength = 1.0f;
        
        if (m_paramValues.count("radius")) {
            radius = m_paramValues.at("radius").asFloat();
        }
        if (m_paramValues.count("quality")) {
            quality = m_paramValues.at("quality").asInt();
        }
        if (m_paramValues.count("maskStrength")) {
            maskStrength = m_paramValues.at("maskStrength").asFloat();
        }
        
        Logger::getInstance().logMessage("Using parameters: radius=%.2f, quality=%d, maskStrength=%.2f", 
        ...
        Logger::getInstance().logMessage("WARNING: Unknown effect name: %s", effectName.c_str());
    }
}