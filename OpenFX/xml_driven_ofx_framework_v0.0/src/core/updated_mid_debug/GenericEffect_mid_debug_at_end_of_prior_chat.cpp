...
void GenericEffect::setupAndProcess(const OFX::RenderArguments& p_Args) {
    ...
    try {
        // Get the dst image (same pattern as Resolve examples)
        ...
        // Get the src image (same pattern as Resolve examples)
        OFX::Clip* srcClip = m_dynamicClips["source"];
        if (srcClip && srcClip->isConnected()) {
            std::unique_ptr<OFX::Image> src(srcClip->fetchImage(p_Args.time));

            const OFX::BitDepthEnum srcBitDepth = src->getPixelDepth();
            const OFX::PixelComponentEnum srcComponents = src->getPixelComponents();

            // Check to see if the bit depth and number of components are the same (same as Resolve examples)
            if ((srcBitDepth != dstBitDepth) || (srcComponents != dstComponents)) {
                Logger::getInstance().logMessage("ERROR: Bit depth or component mismatch");
                OFX::throwSuiteStatusException(kOfxStatErrValue);
                return;
            }

            Logger::getInstance().logMessage("Images compatible, bit depth: %d, components: %d", (int)dstBitDepth, (int)dstComponents);
            
            // TODO: This is where we'll add proper image processing
            // For now, just log that we would process here
            Logger::getInstance().logMessage("Would process images here - implementing safe pass-through next");
        } else {
            Logger::getInstance().logMessage("ERROR: Source clip not connected");
        }
